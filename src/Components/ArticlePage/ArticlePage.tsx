...
import { Button, CardBody, CardTitle, Col, Container, Row } from "react-bootstrap";
...
export const ArticlePage = () =>{
                       ...
                       await api('/api/article/search/', 'post',{
                            ...
                            .then((res: ApiResponse | undefined)=>{
                                ...
                                const fetchArticles: ArticleType[] =
                                    res?.data.map((article: ApiArticleDto)=>{
                    ...
                    <Container className='w-100 d-flex wrapperFeature'>
                        <Row className='rowButtons'>
                            <Col className='p-0 m-0 buttons'>
                                <button onClick={description} className='buttonOpis'>Opis proizvoda</button>
                                <button onClick={similarAndDescriptionBtn} className='buttonOpis'>Slični proizvodi</button>
                            </Col>
                        </Row>
                    </Container>
                )
    ...
    const handlePrevPhoto = () => {
        setCurrentPhotoIndex((prevIndex) =>
            prevIndex === 0 ? (articleState.article?.photos?.length || 1) - 1 : prevIndex - 1
        );
    };

    const handleNextPhoto = () => {
        setCurrentPhotoIndex((prevIndex) =>
            prevIndex === (articleState.article?.photos?.length || 1) - 1 ? 0 : prevIndex + 1
        );
    };
                        ...
                        <div className='w-100 p-3 d-flex flex-column justify-content-center align-items-center'>
                            <div className='d-flex kolicina border justify-content-center align-items-center mb-1'>
                                <div className='w-100 d-flex justify-content-center'>
                                    <button disabled={articleState.article?.status === 'visible'} onClick={decrementQuantity} className='w-100 border'>-</button>
                                </div>
                                <div className='w-100 d-flex justify-content-center text-center align-items-center'>
                                    <p className='p-0 m-0'>{quantity}</p>
                                </div>
                                <div className='w-100 d-flex justify-content-center'>
                                    <button disabled={articleState.article?.status === 'visible'} onClick={incrementQuantity} className='w-100 border'>+</button>
                                </div>
                            </div>
